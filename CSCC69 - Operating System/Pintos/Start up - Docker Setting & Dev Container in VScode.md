...
- 定义如何构建基础开发环境，包括安装依赖、设置环境变量、默认工作目录等
...
- 如构建出的镜像是 arm64 架构，而在运行阶段使用 `--platform=linux/amd64` 启动容器，则 VSCode 会找不到架构匹配的镜像，即使镜像名相同，也会报出:
...
- 如果要使用 `-qemu` 要单独安装`qemu-system-i386`，可以直接使用`apt-get update && apt-get install -y qemu-system-i386`在容器内安装，或者如果之后要rebuild 可以放在Dockerfile里(试过了可以的！)
